...
namespace ortc {
  ...
  json getRtpCapabilities(json extendedRtpCapabilities) {
    ...
    for (auto capCodec : extendedRtpCapabilities.at("codecs")) {
      ...
      // if kind = video  Add RTX codec.
      if (capCodec.count("recvRtxPayloadType") && (capCodec.at("kind") == "video")) {
      ...
      for (auto aFb : codecA.at("rtcpFeedback")) {
        if (codecB.count("rtcpFeedback")) {
          ...
          auto search = std::find_if(codecBRtcpFeedback.begin(), codecBRtcpFeedback.end(), [&](auto bFb) {
              ...
              bool isTypeEqual = bFb.at("type") == aFb.at("type");
              
              bool isParameterEqual = false;
              
              auto b_cout = bFb.count("parameter");
              auto a_cout = aFb.count("parameter");
              isParameterEqual = !b_cout && !a_cout;
              
              if (b_cout && a_cout) {
                  isParameterEqual = bFb.at("parameter") == aFb.at("parameter");
              }
              return isTypeEqual && isParameterEqual;
              return false;
          });
...
namespace commonUtils_cpp {
  json extractRtpCapabilities(json sdpObj) {
    // auto dumped = sdpObj.dump();
    // log("SDP OJB" + dumped);
    // Map of RtpCodecParameters indexed by payload type.
      std::map<int, json> codecsMap_o;
      std::map<int, json> &codecsMap = codecsMap_o;
    ...
    for (auto& m : sdpObj.at("media")) {
      ...
      for (auto &rtp : m.at("rtp")) {
        ...
        if (rtp.count("encoding") > 0) {
          channels = rtp.at("encoding");
            if (channels.is_string()){
                string str = channels;
                channels = atoi(str.c_str());
            }
        }
        json codec = {
          {"name", codecName},
      ...
      for (auto &fmtp : fmtps) {
        json parameters = sdptransform::parseParams(fmtp.at("config").get<string>());
        auto search = codecsMap.find(fmtp.at("payload").get<int>());
          json &codec = search->second;
        if (search == codecsMap.end()) {
          continue;
        } else {
          json *p_codec_parameters = &(codec.at("parameters"));
            *p_codec_parameters = parameters;
...
json getExtendedRtpCapabilities(json sdpObj, json roomCapabilities) {
  auto clientCapabilities = commonUtils_cpp::extractRtpCapabilities(sdpObj);
...
json getEffectiveClientRtpCapabilities(json sdpObj, json roomCapabilities) {
